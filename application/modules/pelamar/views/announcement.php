				...
				<div class="card " style="overflow-x: scroll;">
					...
					<div class="card-body ">
						<?php if ($user_data->status == -1): ?>
							<h1 class="text-primary">Belum dinilai</h1>
						<?php elseif ($user_data->status == 0): ?>
							<h1 class="text-danger">Tidak Lulus</h1>
						<?php elseif ($user_data->status >= 1 && count($ranked) > 0 && $ranked[0]['user_id'] == $user->id): ?>
							<h1 class="text-success">Lulus</h1>
						<?php else: ?>
							<h1 class="text-success">Lulus Administrasi</h1>
						<?php endif; ?>
